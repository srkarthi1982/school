...
from app.modules.course_selection_material.services import (
    ...
    create_file_record,
    create_folder,
    create_library_material_for_cs_file,
    delete_file,
    delete_folder,
    duplicate_file,
    get_file,
    ...
    set_material_status,
    storage,
    sync_delete_library_material,
    sync_update_library_material,
)

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/course-selection-materials",
                   tags=["Course Selection Material"])
...
def get_material(
    ...
    return ok(serialize_material(db, course))
...
def mark_material_complete(
    ...
    course = get_or_raise_course(db, material_id)
    set_material_status(db, course, "complete", user.id)
    return ok(
        MaterialStatusResponse(
...
def unmark_material_complete(
    ...
    course = get_or_raise_course(db, material_id)
    set_material_status(db, course, "incomplete", user.id)
    return ok(
        MaterialStatusResponse(
            material_id=course.id,
            status="incomplete",
...
# ---------------------------------------------------------------------------
# Lessons (material is organised under the master's lessons)
# ---------------------------------------------------------------------------
...
def get_lessons(
    ...
    course = get_or_raise_course(db, material_id)
    items = list_material_lessons(db, course)
...
def get_files(
    ...
    files_with_lib, total = list_files(
        db, material_id, folder_id, limit, offset, lesson_id)
    items = [
...
async def upload_file(
    ...
    try:
        lib_material = create_library_material_for_cs_file(
            db, file_record, total_pages=total_pages)
        try:
            ...
            get_or_create_summary_by_type(db, "course", lib_material.id)
            trigger_summarize(lib_material.id)
            library_material_id = lib_material.id
        except Exception:
            logger.exception(
                "Failed to register CSM file for summarization %s", file_record.id)
    except Exception:
        logger.exception(
            "Failed to sync CSM upload to Library %s", file_record.id)

    return ok(serialize_upload(file_record, library_material_id=library_material_id))
...
def patch_file(
    ...
    sync_update_library_material(db, file_record)
...
def get_shared_files(
    ...
    files_with_lib, total = list_shared_files(
        db, material_id, exclude_lesson_id, limit, offset)
    items = []
    for f, lib_id in files_with_lib:
        item = serialize_shared_file(f, library_material_id=lib_id)
