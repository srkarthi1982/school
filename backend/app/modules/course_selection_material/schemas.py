...
class MaterialResponse(BaseModel):
    ...
    root_file_count: int
    # True when any material differs from the master; ``course_modified`` is the
    # whole-course (non-lesson) scope specifically.
    modified: bool = False
...
class MaterialLessonResponse(BaseModel):
    """One lesson the instance's material is organised under.
    ...
    instance material files uploaded under this lesson.
    """
...
class SharedMaterialFile(BaseModel):
    """A material file from another lesson within the same instance material."""
