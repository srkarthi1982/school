...
class EvaluationLessonResponse(BaseModel):
    """One lesson the course's evaluation is organised under.

    Sourced live from the Course Information → Lesson Creation step
    (``course_info_lesson_creation_lessons``); ``quiz_count`` is the number of
...
class EvaluationQuizResponse(BaseModel):
    ...
    id: int  # association id (EvaluationLessonQuiz.id)
    quiz_id: int
    ...
    question_count: int
    order_index: int
    # Per-association assessment configuration.
...
class UpdateQuizAssociationRequest(BaseModel):
    """Update the per-association assessment configuration. The quiz and its
    lesson/course scope are fixed at creation, so only these fields change."""
...
class EvaluationFormResponse(BaseModel):
    ...
    id: int  # association id (EvaluationLessonForm.id)
    form_id: int
