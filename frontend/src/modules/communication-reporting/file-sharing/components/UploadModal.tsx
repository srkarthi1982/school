...
import { HiOutlineXMark, HiOutlineCloudArrowUp, HiOutlineExclamationCircle } from 'react-icons/hi2'
...
export default function UploadModal({ open, onClose, onUpload, title, dropLabel, uploading }: Props) {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4">
          <div
            ...
            onClick={() => document.getElementById('file-share-input')?.click()}
          >
          ...
          {files.length > 0 && (
            <div className="mt-3 flex flex-col gap-2 max-h-48 overflow-y-auto thin-scrollbar-light">
              {files.map((entry, index) => (
                ...
                >
                  <span className="text-[12px] truncate flex-1" style={{ color: entry.error ? 'rgb(239 68 68)' : 'var(--text-primary)' }}>
                    {entry.file.name}
                  </span>
                  <span className="text-[11px] text-muted shrink-0">{(entry.file.size / (1024 * 1024)).toFixed(1)} MB</span>
                  {entry.error && (
                    <span className="text-[11px] text-red-500 shrink-0" title={entry.error === 'fileTypeNotAllowed' ? 'File type not allowed' : 'File exceeds 100 MB limit'}>
