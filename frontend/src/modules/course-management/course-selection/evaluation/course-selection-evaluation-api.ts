...
// A form linked to a lesson (or the course) within the instance's Evaluation. A
// plain attachment — no assessment config, unlike quizzes.
...
// Seeds the instance's lesson↔quiz associations from its master on first call.
export const ensureInstanceEvaluation = (courseId: number): Promise<EvaluationDetail> =>
  apiFetch(`/courses/${courseId}/evaluation`, { method: 'POST' })

export const markEvaluationComplete = (evaluationId: number): Promise<EvaluationStatus> =>
  apiFetch(`/course-selection-evaluations/${evaluationId}/complete`, { method: 'POST' })

export const unmarkEvaluationComplete = (evaluationId: number): Promise<EvaluationStatus> =>
  apiFetch(`/course-selection-evaluations/${evaluationId}/incomplete`, { method: 'POST' })
...
export const listEvaluationLessons = (
  ...
  apiFetch(`/course-selection-evaluations/${evaluationId}/lessons`)
...
export const listLessonQuizzes = (
  ...
  return apiFetch(`/course-selection-evaluations/${evaluationId}/quizzes${query}`)
}

export const listAllQuizzes = (
  evaluationId: number,
): Promise<{ items: EvaluationQuiz[]; total: number }> =>
  apiFetch(`/course-selection-evaluations/${evaluationId}/quizzes/all`)
...
export const associateQuiz = (
  ...
  apiFetch(`/course-selection-evaluations/${evaluationId}/quizzes`, {
    method: 'POST',
...
export const updateQuizAssociation = (
  ...
  apiFetch(`/course-selection-evaluations/quizzes/${assocId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      assessment_type: input.assessmentType,
...
export const dissociateQuiz = (assocId: number): Promise<void> =>
  apiFetch(`/course-selection-evaluations/quizzes/${assocId}`, { method: 'DELETE' })
...
export const listEvaluationForms = (
  ...
  const query = lessonId == null ? '' : `?lesson_id=${lessonId}`
  return apiFetch(`/course-selection-evaluations/${evaluationId}/forms${query}`)
}

export const listAllEvaluationForms = (
  evaluationId: number,
): Promise<{ items: EvaluationForm[]; total: number }> =>
  apiFetch(`/course-selection-evaluations/${evaluationId}/forms/all`)
...
export const associateForm = (
  ...
  apiFetch(`/course-selection-evaluations/${evaluationId}/forms`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export const dissociateForm = (linkId: number): Promise<void> =>
  apiFetch(`/course-selection-evaluations/forms/${linkId}`, { method: 'DELETE' })
