...
import type { FormBuilderLesson } from './course-selection-form-api'
import { useCourseSelectionFormStore } from './course-selection-form-store'
...
export default function CourseSelectionFormPage() {
  ...
  useEffect(() => {
    ...
    ensureInstanceFormBuilder(courseId)
      .then((data) => {
        if (cancelled) return
        setFormBuilder(data.id, data.status, data.surveys_completion, data.title, !!data.course_modified)
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      setStatus(resp.status, resp.surveys_completion)
    } finally {
          ...
          {loading && lessons.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-[12.5px] text-muted mb-4 max-w-md">
                You can still associate {kindLabel}s with the whole course above. Lessons come from
                this course&rsquo;s master template (Course Information → Lesson Creation).
              </p>
              <button
                type="button"
                onClick={() => navigate(detailPath)}
