...
import type { FormBuilderLesson, FormBuilderSurvey, SurveyOption } from './course-selection-form-api'
...
export default function CourseSelectionFormSurveyTargetPage() {
  ...
  const formBuilderPath = `/course-management/course-selection/${courseId}/category/surveys`
  ...
  useEffect(() => {
    ...
    ensureInstanceFormBuilder(courseId)
      .then(async (data) => {
        if (cancelled) return
        setFormBuilder(data.id, data.status, data.surveys_completion, data.title)
  ...
  }, [courseId, lessonId, isCourse, lessonFromState, setFormBuilder, setCurrentLessonId])
