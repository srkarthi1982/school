...
import type { MaterialFile, SharedMaterialFile } from './course-selection-material-api'
...
export default function CourseSelectionMaterialFilesPage() {
  ...
  const [libIdMap, setLibIdMap] = useState<Record<string, number | null | undefined>>({})
  const [loadedMaterialId, setLoadedMaterialId] = useState<number | null>(null)
  const [courseTitle, setCourseTitle] = useState('')
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [searchQuery, setSearchQuery] = useState('')

  // Resolve library_material_id per CSM file (the shared-files list doesn't return it)
  useEffect(() => {
    if (!files.length || !loadedMaterialId) return
    // const done = files.map((f) =>
    //   listMaterialFiles(loadedMaterialId, undefined, f.source_lesson_id, 200)
    //     .then((res) => {
    //       const idMap: Record<string, number> = {}
    //       res.items.forEach((item: MaterialFile) => {
    //         if (item.library_material_id != null) {
    //           idMap[item.id] = item.library_material_id
    //         }
    //       })
    //       if (Object.keys(idMap).length > 0) {
    //         setLibIdMap((prev) => ({ ...prev, ...idMap }))
    //       }
    //     })
    //     .catch(() => {}),
    // )
    // void Promise.allSettled(done)
    const idMap: Record<string, number | null | undefined> = {}
    files.forEach(f => {
      idMap[f.id] = f.library_material_id
    })
    setLibIdMap((prev) => ({ ...prev, ...idMap }))
  }, [files, loadedMaterialId])

  const getLibMaterialId = (fileId: string): number | null => libIdMap[fileId] ?? null

  const openInLibrary = (fileId: string) => {
    const libId = libIdMap[fileId]
    if (libId == null) return
    navigate(`/course-management/library/view/${libId}`)
  }
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const material = await ensureInstanceMaterial(courseId)
      setLoadedMaterialId(material.id)
          ...
          <div className="flex flex-col gap-3">
            {filteredFiles.map((file) => (
              <div key={file.id} className="card px-5 py-4 flex items-center gap-4">
                ...
                <button
                  type="button"
                  onClick={() => openInLibrary(file.id)}
                  disabled={!getLibMaterialId(file.id)}
