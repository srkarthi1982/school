...
import type { MaterialFile, MaterialFolder, MaterialLesson } from './course-selection-material-api'
import { useCourseSelectionMaterialStore } from './course-selection-material-store'
...
import MaterialNewFolderModal from '../../material/components/MaterialNewFolderModal'
import MaterialUploadModal from '../../material/components/MaterialUploadModal'
...
export default function CourseSelectionMaterialLessonPage() {
  const { id, lessonId: lessonIdParam } = useParams<{ id: string; lessonId: string }>()
  const courseId = Number(id)
  const lessonId = Number(lessonIdParam)
  const navigate = useNavigate()
  const location = useLocation()
  const materialPath = `/course-management/course-selection/${courseId}/category/material`
  ...
  const {
    ...
    updateFile,
    reset,
  } = useCourseSelectionMaterialStore(
    useShallow((s) => ({
  ...
  const store = useCourseSelectionMaterialStore()
  ...
  // Reset store when leaving so a different lesson/course starts fresh
  useEffect(() => {
    return () => {
      reset()
    }
  ...
  // Resolve (and lazily seed) the material for this course instance, scoped to the lesson
  useEffect(() => {
    ...
    ensureInstanceMaterial(courseId)
      .then(async (data) => {
        if (cancelled) return
        setMaterial(data.id, data.status, data.material_completion)
        if (!lessonFromState) {
  ...
  }, [courseId, lessonId, lessonFromState, setMaterial, setCurrentLessonId])
