...
import type { MaterialLesson } from './course-selection-material-api'
...
export default function CourseSelectionMaterialPage() {
  ...
  const {
    ...
    useShallow((s) => ({
      ...
      status: s.status,
      materialCompletion: s.materialCompletion,
      lessons: s.lessons,
      loading: s.loading,
  ...
  // Once the course is approved, completed categories are locked.
  ...
  // Reset store when leaving so a different course instance starts fresh
  useEffect(() => {
    return () => {
      reset()
    }
  ...
  // Resolve (and lazily seed) the material for this course instance
  useEffect(() => {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      setStatus(resp.status, resp.material_completion)
    } finally {
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="px-4 py-3 border-b border-[var(--border)] bg-[var(--surface)]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold text-muted ml-2">{materialCompletion}%</span>
        </div>
        </div>
      </div>
        ...
        {loading && lessons.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-[12.5px] text-muted mb-4 max-w-md">
              Material is organised by lesson. This course&rsquo;s lessons come from its master
              template&rsquo;s Course Information → Lesson Creation step.
            </p>
            <button
              type="button"
              onClick={() => navigate(detailPath)}
