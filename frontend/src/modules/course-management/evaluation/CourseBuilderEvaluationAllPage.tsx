...
import {
  ensureCourseMasterEvaluation,
  listAllEvaluationForms,
  listAllQuizzes,
...
// A quiz or form, de-duplicated across its lesson/course associations. A single
// quiz/form linked to several lessons appears once, with every association
// listed under ``associations``.
interface AllItem {
  kind: Tab
  refId: number // quiz_id or form_id (the underlying quiz/form, not the link)
  title: string
  status: string
...
export default function CourseBuilderEvaluationAllPage() {
  ...
  const evaluationPath = `${detailPath}/category/evaluation`

  const [tab, setTab] = useState<Tab>('quiz')
  const [courseTitle, setCourseTitle] = useState('')
  const [lessons, setLessons] = useState<EvaluationLesson[]>([])
  const [quizzes, setQuizzes] = useState<EvaluationQuiz[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const ev = await ensureCourseMasterEvaluation(masterId)
      setCourseTitle(ev.title)
      const [lessonRes, quizRes, formRes] = await Promise.all([
        listEvaluationLessons(ev.id),
        listAllQuizzes(ev.id),
        listAllEvaluationForms(ev.id),
      ])
      setLessons(lessonRes.items)
      setQuizzes(quizRes.items)
      setForms(formRes.items)
    } catch (e) {
      setError((e as Error).message || 'Failed to load evaluation')
    } finally {
      setLoading(false)
    }
  }, [masterId])
  ...
  const items: AllItem[] = useMemo(() => {
    // De-duplicate by the underlying quiz/form so a single quiz/form linked to
    ...
    const add = (
      ...
      if (existing) {
        existing.associations.push({ linkId, lessonId })
        return
      }
      byRef.set(refId, {
        kind: tab,
        ...
        status,
        questionCount,
        associations: [{ linkId, lessonId }],
      })
    }
    if (tab === 'quiz') {
      for (const q of quizzes) add(q.quiz_id, q.id, q.lesson_id, q.name, q.status, q.question_count)
    } else {
      for (const f of forms) add(f.form_id, f.id, f.lesson_id, f.title, f.status, f.question_count)
    }
  ...
  // Distinct counts (each quiz/form counted once regardless of associations).
  const distinctQuizCount = useMemo(
    () => new Set(quizzes.map((q) => q.quiz_id)).size,
    [quizzes],
  )
  ...
  const openTarget = (lessonId: number | null) => {
    const target = lessonId == null ? 'course' : `lesson/${lessonId}`
    navigate(`${evaluationPath}/${target}`)
  }

  const kindLabel = tab === 'quiz' ? 'quiz' : 'form'
  const kindPlural = tab === 'quiz' ? 'quizzes' : 'forms'
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center gap-1 px-4 pt-3 border-b border-[var(--border)]">
        <TabButton
          active={tab === 'quiz'}
          onClick={() => setTab('quiz')}
          icon={<HiOutlineQuestionMarkCircle className="text-[14px]" />}
          label={`Quizzes (${distinctQuizCount})`}
        />
        <TabButton
          active={tab === 'form'}
          onClick={() => setTab('form')}
      ...
      <div className="flex items-center gap-2 px-6 py-4 border-b border-[var(--border)]">
        ...
        <span className="text-[13px] font-bold text-primary capitalize">{kindPlural}</span>
      ...
      <div className="px-6 py-3 border-b border-[var(--border)]">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={`Search ${kindPlural} by title or lesson…`}
            value={searchQuery}
      ...
      <div className="flex-1 overflow-y-auto thin-scrollbar-light px-6 py-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-[14px] font-semibold text-primary mb-1">No {kindPlural} yet</p>
            <p className="text-[12.5px] text-muted mb-4 max-w-md">
              No {kindPlural} have been associated with this course or its lessons yet. Open the
              course or a lesson to assign {kindPlural}.
            </p>
            <button
              type="button"
              onClick={() => navigate(evaluationPath)}
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <p className="text-[13px] font-semibold text-primary mb-1">
              No {kindPlural} match &ldquo;{searchQuery}&rdquo;
            </p>
