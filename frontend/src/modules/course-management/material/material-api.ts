...
export const ensureCourseMasterMaterial = (masterId: number): Promise<MaterialDetail> =>
  apiFetch(`/course-masters/${masterId}/material`, { method: 'POST' })
...
export const markMaterialComplete = (materialId: number): Promise<MaterialStatus> =>
  apiFetch(`/materials/${materialId}/complete`, { method: 'POST' })

export const unmarkMaterialComplete = (materialId: number): Promise<MaterialStatus> =>
  apiFetch(`/materials/${materialId}/incomplete`, { method: 'POST' })
...
export const listMaterialLessons = (
  ...
  apiFetch(`/materials/${materialId}/lessons`)
...
export const listMaterialFolders = (
  ...
  return apiFetch(`/materials/${materialId}/folders${qs}`)
}
...
export const createMaterialFolder = (
  ...
  apiFetch(`/materials/${materialId}/folders`, {
    method: 'POST',
...
export const renameMaterialFolder = (folderId: string, name: string): Promise<MaterialFolder> =>
  apiFetch(`/materials/folders/${folderId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export const deleteMaterialFolder = (folderId: string): Promise<void> =>
  apiFetch(`/materials/folders/${folderId}`, { method: 'DELETE' })

export const getMaterialBreadcrumb = (folderId: string): Promise<MaterialBreadcrumbItem[]> =>
  apiFetch(`/materials/folders/${folderId}/breadcrumb`)
...
export const listMaterialFiles = (
  ...
  return apiFetch(`/materials/${materialId}/files?${params.toString()}`)
}
...
export const uploadMaterialFile = (
  ...
  if (lessonId != null) params.set('lesson_id', String(lessonId))
  const qs = params.toString() ? `?${params.toString()}` : ''
  return apiFetch(`/materials/${materialId}/files${qs}`, {
    method: 'POST',
    body: formData,
  })
}
...
export const downloadMaterialFile = async (fileId: string, filename: string): Promise<void> => {
  const token = localStorage.getItem('access_token')
  const res = await fetch(`/api/v1/materials/files/${fileId}/download`, {
...
export const renameMaterialFile = (fileId: string, filename: string): Promise<MaterialFile> =>
  apiFetch(`/materials/files/${fileId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export const deleteMaterialFile = (fileId: string): Promise<void> =>
  apiFetch(`/materials/files/${fileId}`, { method: 'DELETE' })
...
export const listSharedMaterialFiles = (
  ...
  params.set('limit', String(limit))
  params.set('offset', String(offset))
  return apiFetch(`/materials/${materialId}/shared-files?${params.toString()}`)
}
...
export const addExistingMaterialFile = (
  ...
  apiFetch(`/materials/${materialId}/add-material`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
