...
// A form or survey, de-duplicated across its lesson/course associations. A
// single form/survey linked to several lessons appears once, with every
// association listed under ``associations``.
interface AllItem {
  kind: Tab
...
export default function CourseBuilderFormBuilderAllPage() {
  ...
  const formBuilderPath = `${detailPath}/category/surveys`
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const fb = await ensureCourseMasterFormBuilder(masterId)
      ...
      setError((e as Error).message || 'Failed to load Form Builder')
    } finally {
      setLoading(false)
    }
  }, [masterId])
  ...
  const items: AllItem[] = useMemo(() => {
    // De-duplicate by the underlying form/survey so a single form/survey linked
    // to multiple lessons shows once, with all its associations collected.
  ...
  // Distinct counts (each form/survey counted once, regardless of how many
  // lessons it is associated with) for the tab labels.
  ...
  const openTarget = (kind: Tab, lessonId: number | null) => {
    ...
    navigate(`${formBuilderPath}/${kind}/${target}`)
  }
