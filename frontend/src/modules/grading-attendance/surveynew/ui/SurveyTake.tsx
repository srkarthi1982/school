...
import { StudentResponse, Survey, SurveyQuestion } from "../types/survey";
...
export default function SurveyTake({ isOpen, onClose, surveyId, studentId, mode = 'take', lessonCtx, onReturnToLesson }: Props) {

    const isPreview = mode === 'preview'


    //  console.log('selectedSurveyId from surveyTake',selectedSurveyId,surveys)
    const { startResponse, getResponseBySurveyAndStudent, saveProgress, finishResponse } = useStudentStore();

    //  const survey = surveys.find(s => s.id === surveyId || s.id == selectedSurveyId)
    ...
    const { surveys, selectedSurveyId } = useSurveyStore(
        useShallow(s => ({
            surveys: s.surveys,
    ...
    useEffect(() => {
        const currentSurvey = surveys.find(s => s.id === surveyId || s.id == selectedSurveyId)
        setSurvey(currentSurvey)
        setCurrentQuestion(currentSurvey?.questions[currentIndex])
    ...
    useEffect(() => {
        if (!isOpen || !survey || surveyId === 0 || isPreview) return
        ...
        const existing = getResponseBySurveyAndStudent(surveyId, studentId)
        if (existing) {
            applyResponse(existing)
        } else if (mode === 'take') {
            // No response yet (e.g. a freshly assigned survey) — create one and
            // apply it once the API call resolves, otherwise `response` stays
            // null and "Finish" can't save anything.
            startResponse(surveyId, studentId).then(resp => {
                if (resp) applyResponse(resp)
            })
        }
    }, [isOpen, survey, surveyId, studentId, mode, isPreview, getResponseBySurveyAndStudent, startResponse])
    ...
    //     () => (hasSurveyTake ? surveys.filter((s) => s.status === 'published') : surveys),
    //     [surveys, hasSurveyTake],
    ...
    const calculateOverAllGrade = (survey: Survey, answers: Record<string, any>): number => {
        let totalScore = 0
    ...
    const handleFinish = async () => {
        ...
        const grade = calculateOverAllGrade(survey, answers);
        ...
        // If sent here from a lesson, mark it complete (no-op otherwise). Use the
        // captured ctx: the URL params were already stripped on arrival.
        void completeLessonContentIfLinked('survey', surveyId, lessonCtx)
                        ...
                        <div className="space-y-3">
                            ...
                            {currentQuestion?.type === 'rating_with_text' && (
                                <div className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-4">
                                        {/* <span>this is the value: {JSON.stringify(answers)}</span> */}
                                        <input
                                            type="range"
                                            min={1}
                                            max={10}
